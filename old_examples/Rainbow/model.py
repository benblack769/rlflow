...
class NoisyLinear(nn.Module):
  ...
  def reset_parameters(self):
    ...
    self.weight_sigma.data[:] = (self.std_init / math.sqrt(self.in_features))
...
class DQN(nn.Module):
  def __init__(self, args, obs_space, action_space):
    ...
    in_d = obs_space.shape[0]
    if args.architecture == 'canonical':
      self.convs = nn.Sequential(nn.Conv2d(in_d, 32, 8, stride=4, padding=0), nn.ReLU(),
