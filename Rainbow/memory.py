...
Transition_dtype = np.dtype([('timestep', np.int32), ('state', np.uint8, (84, 84)), ('action', np.int32), ('reward', np.float32), ('nonterminal', np.uint8_)])
...
class ReplayMemory():
  ...
  def _get_transitions(self, idxs):
    ...
    blank_mask = np.zeros_like(transitions_firsts, dtype=np.uint8_)
